...
public partial class FrmMain : Form
{
	...
	private void FrmMain_Load(object sender, EventArgs e)
	{
		...
		// Forwardチェックボックスの初期設定
		ChkForwadingInit();

		// Burst Data チェックボックスの初期化
		ChkBurstInit();
	...
	protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
	{
    	...
    	{
			// クローズボタンを押したのと同じ動作とする
			BtnQuit_Click(null, EventArgs.Empty);

        	// Alt + F4 を無効化
	        return true; // キーイベントを処理済みとし、システムに渡さない
    	}
	...
	///********************************************
	/// タイトルバーのダブルクリックによる最大化を規制する
	///********************************************
	protected override void WndProc(ref Message m)
	{
    	const int WM_NCLBUTTONDBLCLK = 0x00A3; // タイトルバーのダブルクリックメッセージ
	...
	//***********************************************************
	//	Burst対応チェックボックスの初期値設定と変更の保存
	//***********************************************************

	// 通常はアンチェックを設定
	private void ChkBurstInit()
	{
			ChkBurst.Checked = false;
	}
	
	// チェックボックスを変更したら状況をSettingsに反映
	private void ChkBurst_CheckedChanged(object sender, EventArgs e)
	{
		// 選択した衛星名を設定パネルの TxtLastUsed.text に反映させる
		if (ChkBurst.Checked == true)
		{
			ChkForwarding.Checked = false;
			Log.Information("Burst Data set to YES.");
		}
	}
...
// Debug end  */
	...
	// Burst DATA をセパレータで分割してワンパケット化する
	...
	/// Telemetryデータの処理
	/// 
	private void Forwarding(byte[] recvData)
	{
	...
	//
	// 転送のためにPythonのrequestsを使った送信コマンドを起動する
	//
